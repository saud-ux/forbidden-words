...
// ── Session persistence — survives a full page reload / refresh ────────────
// The reconnect tokens used to live only in memory, so hitting F5 killed the
// game even though the server keeps the room alive for a grace period. We now
// mirror the session to sessionStorage and re-hydrate it on load, BEFORE the
// socket connects, so the `connect` handler below can transparently rejoin.
...
socket.on('game:ended', ({ scoreboard: sb }) => {
  clearSession(); // game over — don't auto-rejoin a deleted room on reload
...
socket.on('connect',    () => {
  hideBanner();
  // No saved session → genuine first connect on the landing screen, nothing to do.
  ...
  if (state.role === 'host') {
    ...
    socket.emit('host:reconnect', { code: state.roomCode, hostToken: state.hostToken }, res => {
      ...
      // After a FULL page reload we're still on the landing screen — restore the
      // host dashboard. Its contents repopulate from the server's resync events.
    ...
    socket.emit('player:join', { code: state.roomCode, name: state.playerName }, res => {
      ...
      // Land on the right screen; if a round is live and we're the explainer the
      // server's explainer:card / round:start resync will flip us automatically.
      showScreen(state.role === 'explainer' ? 'screen-explainer' : 'screen-guesser');
    });
  }
});

// ── Styled confirm dialog (replaces the bare browser confirm) ──────────────
let _confirmCb = null;
...
// ── Player leaves the session (Exit button on the player screens) ──────────
// Confirms first, tells the server to fully remove the player, clears the saved
// session, then reloads — which lands back on the code + name entry screen.
function leaveSession() {
  askConfirm('هل أنت متأكد من الخروج؟', () => {
    ...
    setTimeout(finish, 600); // fallback in case the ack never arrives
  });
}
