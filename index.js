...
const {
  ...
  joinRoom, leaveRoom, removePlayer, getActivePlayers,
...
io.on('connection', (socket) => {
  ...
  // ── Player leaves the room on purpose (Exit button) ──────────────────────
  // Unlike a disconnect, this fully removes the player so they free their name
  // and vanish from the scoreboard immediately.
  socket.on('player:leave', ({ code }, cb) => {
    const room = getRoom(code);
    if (!room) return cb?.({ ok: true });

    const wasExplainer = getExplainer(room)?.socketId === socket.id;
    const player = leaveRoom(code, socket.id);
    socket.leave(code);

    if (player) {
      // If the explainer bailed mid-round, end the round cleanly (the seat was
      // already cleared by leaveRoom) so the host can reassign.
      if (wasExplainer && room.phase === 'round') endRound(room, 'explainer_disconnect');
      emitLobby(room);
